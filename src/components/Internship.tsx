...
export default function Internship() {
  ...
  type InternshipType = 'long' | 'short';
  ...
  const [activeTab, setActiveTab] = useState<InternshipType>('long');
  const filtered = useMemo(
    () => internships.filter((i) => i.type === activeTab),
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.tabs} role="tablist" aria-label="インターン種別">
        <button
          role="tab"
          aria-selected={activeTab === 'long'}
          aria-controls="long-panel"
          className={`${styles.tab} ${activeTab === 'long' ? styles.tabActive : ''}`}
          onClick={() => setActiveTab('long')}
        >
          就業型インターン
          <span className={styles.tabCount}>
            {internships.filter(i => i.type === 'long').length}
          </span>
        </button>
        <button
          role="tab"
          aria-selected={activeTab === 'short'}
          aria-controls="short-panel"
          className={`${styles.tab} ${activeTab === 'short' ? styles.tabActive : ''}`}
          onClick={() => setActiveTab('short')}
        >
          短期インターン
          <span className={styles.tabCount}>
            {internships.filter(i => i.type === 'short').length}
          </span>
        </button>
      </div>

      <div
        id={activeTab === 'long' ? 'long-panel' : 'short-panel'}
        role="tabpanel"
        className={styles.grid}
      >
        {filtered.map((internship, index) => (
          <div
            key={`${internship.slug}-${index}`}
            className={styles.card}
            onClick={() => router.push(`/internships/${internship.slug}`)}
            tabIndex={0}
            role="button"
            aria-label={`${internship.company} の詳細へ`}
          >
            ...
            <div className={styles.cardBody}>
              ...
              <div className={styles.technologies}>
                ...
                <div className={styles.techTags}>
                  {internship.technologies.slice(0, 4).map((tech, techIndex) => (
